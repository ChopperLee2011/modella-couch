 ...
 * @api public
 */
sync.createDesignDoc = function (name, mapFunction, cb) {
    let db = this.db;
    let designDoc = {
        _id: '_design/' + name,
        views: {}
    ...
    console.info('designDoc.views[name]', designDoc.views[name]);
    db.post(designDoc, function (err, res) {
        console.info('res', res);
        if (err) return cb(err);
        return cb(null, res);
    });
    //TODO: put method run error ,use post for now
 ...
 * @api public
 */
sync.query = function (name, options, cb) {
    options = options || {};
    const db = this.db;
    let collection = [];
    options.include_docs = true;
    db.query(name, options, (err, res) => {
        if (err) return cb(err);
        res.rows.forEach((doc)=> {
            collection.push(new this(doc.doc));
        });
        return cb(null, collection);
    });
};
